...
const towerManager = {
    ...
    run: function() {
        ...
        const myRoom = Game.rooms[params.myRoomName];

        const hostiles = myRoom.find(FIND_HOSTILE_CREEPS, {
            filter: (hostile) => {
                return params.allies.indexOf(hostile.owner.username) === -1;
            }
        });
        const towers = myRoom.find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_TOWER}});
